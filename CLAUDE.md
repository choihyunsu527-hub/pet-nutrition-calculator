...
## Git 작업 흐름 (1인 프로젝트)
- **feature branch / PR을 만들지 않는다.** 개발 완료 후 `main`에 직접 commit / push 한다.
- push 전에 반드시 `node --test`(143케이스)를 실행해 전부 통과한 것을 확인한 뒤 push 한다.
- 로컬 `.git/hooks/pre-push` 훅이 `main` push 직전에 `node --test`를 다시 돌려, 실패 시 push를 차단한다(안전장치). 훅은 저장소에 포함되지 않는 로컬 파일이다.
- 커밋 대상은 작업한 파일만 스테이징한다. untracked 자료 파일(`*.xlsx`, `레시피/` 등)은 커밋하지 않는다.
